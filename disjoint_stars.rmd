...
welcome. You can submit an issue or pull request on this project's
[Github page][project-github-page].
...
redundant) example: the 4-tuple `(1, 4, 2, 3)` corresponds to the permutation
1423.
...
for perm in perms:
    print(perm)
```
# Generating Matching Seuqences of Star Forests

## Code

```{python}
# Given a star forest (sf), this function returns its matching sequence in the
# format that is used as input for gen_perm.
#
# sf is expressed as an array of the form [s_1, s_2, ..., s_n], where s_k is
...
def stars_match(sf):
    ...
    for i in range(len(sf) + 1):
        ...
        m = 0
        for comb in itertools.combinations(range(len(sf)), i):
            m = m + functools.reduce(mul, [sf[index] for index in comb], 1)
        ms.append(m)
    return ms
```
...
First, we try some well-understood star forests.
...
# Iterating over a Certain Family of Star Forests

We would like to count the matching permutations realizable by the family of
star forests with matching number $n$. As of yet, we have not shown that
checking any particular finite subset of this family yields all realizable
permutations. So for now, we iterate over all star forests with matching number
$n$ such that each constituent star has at most $n$ edges, and we obtain a
lower bound.
...
# forests that match the above description. The star forests in the output are
# expressed in the form that stars_match accepts.
def iter_stars(n):
    stars = []
    ...
    # of star forests we are interested in: interpret c_k as the number of
...
```{python}
sf_list = iter_stars(2)
for sf in sf_list:
    print(sf)
```

```{python}
sf_list = iter_stars(3)
for sf in sf_list:
    print(sf)
```
